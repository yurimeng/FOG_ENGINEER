...
> **数据源 / Source of truth：** 本文全部产品参数以 [[PRODUCT_SPEC_BASELINE]]（六 SKU 规格基准表 v2.0）为准 —— **本文任一数值与基准表冲突时，以基准表为准**。本 RA 对应 SKU：**I200C20**。 ^mdc-99750441a3
>
> **置信度标注规则见 [[UNCONFIRMED_Convention]] §2：** ✅ 已确认 · 🔶 derived 我方推导 · ⏳ #unconfirmed 待证实 · ⛔ conflict 源冲突。带 ⏳ / ⛔ 的行**不得对客外发**（[[UNCONFIRMED_Convention]] §5）。

---

## ⛔ CONFLICT · 未裁定 —— 「I200C20 这个产品是什么」两套口径并存 ^mdc-f3e9fcab4a
...
| 口径 | 内容 | 来源 |
|---|---|---|
| **口径 A —— 本 RA 现载（标配 2 槽 + 预留 2 位）** | **标配交付 2×I50TS = 100 kW**，箱内预留 2 个扩展位（B 排，预留管路走位与配电支管），现场吊装扩容至满配 4 槽 200 kW | 本文 §1 / §2 / §4 / §5.1 / §5.3 / §9（RA-003 v1.0，2026-07-12） ^mdc-b6cc833c85 |
| **口径 B —— 站点侧与基准表现载（满配 4 槽出售）** | **4× [[I50TS]] = 200 kW 满配**，无"标配 2 槽"一说；无风冷柜 | 站点 Designer `Immer/src/data/containers/ac20.json` · 站点已发布的 Reference Design 页（**已对客发布**）· [[PRODUCT_SPEC_BASELINE]] §2.1 ^mdc-44d6d0843c |
...
> 相关但**不是同一条**：基准表 §2.1 记 I200C20 **无风冷柜**、并已就"交换机装在哪里"开出未裁定冲突 **C-4**（Layout Planner 裁）；而本文 §4 载「48U 风冷机柜 × 1（网络/管理设备）」、§3 载「机内 2× QM9700 交换机（Tank 间隙立式安装）」—— 本文内部亦不自洽。一并提交 C-4 处理，本次不改。 ^mdc-708de268e8
...
| 项目 | 内容 |
|------|------|
| IT 容量 | **0.2MW（200kW，满配 4× I50TS（原 A32））** ⛔ **产品定义未裁定，见文首 CONFLICT 块** ^mdc-bfadbf3576 |
| 标配交付 | **2×I50TS = 100kW**，预留 2 个扩展位 ⛔ **产品定义未裁定，见文首 CONFLICT 块**（站点侧按满配 4 槽 200 kW 出售，无「标配 2 槽」一说） ^mdc-6142c43320 |
| 产品形态 | 1× AC20（20ft IT 集装箱）+ 1× 20ft 冷源框架 ^mdc-7a8183834f |
...
| 项目 | 标配（2×I50TS）| 满配（4×I50TS）| 说明 ^mdc-d3b161cd44 |
|------|------------|------------|------|
| **IT 负载 / IT Load** | **100kW** | **200kW** | 服务器、GPU 实际消耗，**不含**冷却与配电损耗（I50TS 单柜 50kW Max / 45kW 推荐）。⛔ 标配 / 满配两栏本身即上方产品定义冲突的一部分 ^mdc-25826fca8f |
...
> ⚠️ 服务器与 GPU 数量取决于实际 GPU TDP。若客户 IT 需求 >200kW，请升级至 RA-001（0.4MW，AC40）。 ^mdc-f26e9674e1

---

## 4. 产品配置

本方案使用 **1× AC20 + 1× 20ft 冷源框架**（端对端拼接，整线长度 ≈ 2×6.058m）： ^mdc-ae336d5feb

| 项目 | AC20 参数 ^mdc-da54f1ff08 |
|------|-----------|
| IT 容量 | 200kW 满配（4×50kW I50TS Tank 槽位） ^mdc-65a6cdcfbe |
| 标配 Tank | 2×I50TS（T 排），2 个扩展位（B 排，预留管路走位） ^mdc-2235847e26 |
...
> All-in-One 边界：与 RA-001（AC40 需现场配套 Hybrid Cooling System）不同，本方案冷源随整线出厂、管路预连，现场无冷却侧施工。 ^mdc-8e921b8e17
...
| Tank 型号 | I50TS，50kW Max / 45kW 推荐 ^mdc-95bdfd63cb |
...
> ⛔ **conflict** ** —— 设施水温位与基准表不一致，本次不自行统一。** [[PRODUCT_SPEC_BASELINE]] §2.2 只披露了 **I400C45 的 32 / 37 °C**，**I200C20 一栏本身即为 ⛔ **conflict****；[[RA-001_Immersion_0.4MW|RA-001]] 另载 32–35 / 35–38 °C。加上本文的 **35 / 42–45 °C（油侧）与 31–33 / 38–40 °C（FWS）**，vault 内浸没线设施水温位目前**至少四套并存**且未经裁定，**本次只标注、不合并**。等 Cooling Engineer 确认 I200C20 油侧与 FWS 温位，预期 TBD。 ^mdc-9ec0383499
>
> 注：2026-08-30 裁定 1（外冷源 32–36 °C → CDU +4 °C → GPU 36–40 °C）**只适用 L1800C45 / L450C20 两个双环路液冷 SKU**，不适用本浸没 SKU，故本节未按该链改写。§5.2 的 Chiller 选型计算与 §5.3 冷量匹配计算过程均未改动。 ^mdc-cc6d2d0fc5
...
> **暖水工况说明：** 表列 100% 自然冷却温度为 IDC 低温水工况。浸没方案 FWS 供水 31–33°C（暖水），实际 100% 自然冷却窗口大幅上移：环境 ≤25°C 可全自然冷却，>28°C 机械制冷介入（对齐 I50TS Working Scenario）。 ^mdc-bddbb3df5f

### 5.3 冷量匹配

| 配置 | IT 负载 | 冷源能力 | 裕度 |
|------|--------|---------|------|
| 标配 2×I50TS | 100kW | 262kW（IDC①）| ~160% ^mdc-e085d5c5d5 |
| 满配 4×I50TS | 200kW | 262kW（IDC①）| ~31%（含风冷辅助与损耗后仍 >20%） ^mdc-3a0cb400c3 |
...
| 功率路径 | Grid → (BESS 可选) → Switchgear → PDC1 → I50TS Tanks / 风冷机柜 ^mdc-bdff7659c0 |
| 冷源供电 | Chiller 独立回路（最大 196A），与 IT 配电分开 |
| UPS | **无内置**（AC20 设计取消 UPS/UPS BESS）；如需后备，外置客户自备 ^mdc-fd1631f50e |
...
| **UPS** | 无 | 见 §6 |
| **IT Zone（AC20）** | **无内部冗余** | 单台 AC20 独立运行 ^mdc-d398838551 |
...
| 阶段 | 配置 | IT 容量 | 动作 |
|------|------|--------|------|
| 标配交付 | 2×I50TS（T 排）| 100kW | — ^mdc-cb7f855a22 |
| 扩容 | +2×I50TS（B 排预留位）| 200kW | 现场吊装（内置 KBK 行车），接入预留支管/配电 ^mdc-d5d263a64b |
| 超出 0.2MW | — | — | 升级 RA-001（AC40，0.4MW）或 RA-002（DC45，1.2MW） ^mdc-a91efc499b |
...
IT Load:       200kW 满配（标配 2×I50TS = 100kW）⛔ 产品定义未裁定，见文首 CONFLICT 块 ^mdc-f5a2a36203
Total Load:    随 PUE 变化，逐站点用 https://mdcx.org 计算（不给区间）
PUE:           1.0x（逐站点计算，不给固定值）
Product:       AC20（Immersion Container，20ft）+ 20ft Chiller Frame ^mdc-b792de6a4c
...
- I50TS 技术要求：[[I50TS Technical Requirement|KB/IMMERSION/I50TS/I50TS Technical Requirement]] ^mdc-e77f556a97
...
- 同系列（更大容量）：[[RA-001_Immersion_0.4MW|RA-001 / 0.4MW Immersion（AC40）]]、[[RA-002_Liquid_1.2MW|RA-002 / 1.2MW DLC（DC45）]] ^mdc-5f2f89e83c
- Chiller 规格：TICA TASFV-AAF1 系列选型表（080.1，本文 §5.2 摘录）
- 3D 模型：`MDC/AC_3D/AC20.blend`（20ft + Chiller Frame 布局） ^mdc-126bf373a1
...
| 2026-09-20 | 🧭 unconfirmed-051 改为 ⛔ conflict，未裁定赢家 |
