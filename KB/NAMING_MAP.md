...
| — | **L1800C45** | `L1800C45DR220` | Liquid | 1800 | 45ft | shipped | `KB/LIQUID/L1800C45/` |
| — | **L450C20** | `L450C20DR150` | Liquid | 450 | 20ft | shipped | `KB/LIQUID/L450C20/` |
...
| AC20 | **I200C20** | `I200C20ST50` | Immersion | 200 | 20ft | shipped | `KB/IMMERSION/I200C20/` |
| A32 | **I50TS** | `I50TS` | Immersion | 50 (max) | 单槽 | 组件 | `KB/IMMERSION/I50TS/` |

### I50TS 的推导（KB 侧新增，需站点侧确认）

A32 未出现在 `docs/PRODUCT-MATRIX.md` 中。按 `NAMING.md §2.5` 独立槽体正则
`^[LI][1-9][0-9]*T[SD]U?$` 推导：**I**（浸没）+ **50**（IT Load 50 kW Max，见
[[I50TS Technical Requirement]]）+ **T**（Tank 格式，无尺寸段、无密度段）+ **S**（单环路）+ 无 `U`（不含 UPS）
→ **`I50TS`**。
...
- **正文行文**中出现的旧名（DC45 / AC40 / AC45 / A32）在历史档案、供应商往来、Projects 记录里属事实记录，**保留原样**。
- **SVG 图纸内的文字标签**（`I400C40 Layout_v1.svg` 等）仍为旧名，需重出图。
