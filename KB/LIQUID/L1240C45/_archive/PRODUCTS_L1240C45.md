...
> 本文档 (V1.2, 2026-05-09) 已被 [[MDC/KB/LIQUID/L1240C45/PRODUCTS/L1240C45 Tech Spec EN]] / [[L1240C45 Tech Spec CN]] (V1.4, 2026-05-21) 取代。 ^mdc-e9b83598e6
> V1.2 中的"12×FCU 模组 / Hybrid Cooling / 1.2MW Rack CDU / 8×155kW"与 V1.4 的三支路架构冲突。
> **不要** 在新设计或客户交付中引用本文档。
> 归档日期：2026-06-05

---

# DC45 – PowerPod with Direct Liquid Cooling (45ft) ^mdc-51a821e23d
...
DC45 是 45ft 容器规格的直冷液冷（DLC）集装箱，支持 8 × 150kW DLC 机柜，适用于超大算力 AI 集群部署。 ^mdc-d28c108204

标准 IT 容量为 1240kW，支持与 AC40 混合构建 MDC 集群。 ^mdc-f42eaf9b7e
...
> **注：** 9395XR-1500 中的"1500"= 10 × 150kW 模块，与 AC40 使用的 9395XR-600（4 模块）属同一产品系列。 ^mdc-977b940a2a
...
| PUE 设计值 | **取决于环境温度**：<br>• 低温区（干冷器优先）：~1.07–1.15<br>• 高温区（DX 主导）：~1.25–1.35 | 不写固定数字 |
...
DC45 支持**不预装 DLC 机柜**，客户可后续自行安装原厂 OEM 机柜。 ^mdc-f6dc0ab9c5
...
> **注意：** 原厂 OEM 机柜需自行确认与 DC45 液冷管路的兼容性。 ^mdc-d13b5abb8f

### 5.4 水路冗余

- 2N 系统
- DC45 包含 1 个 1.2MW CDU ^mdc-522676af73
...
- UPS 内置于 DC45，提供约 8 分钟 UPS电池后备 ^mdc-674a2348dc
- UPS电池：3×93LiG2，内置于 DC45（与 BESS 电池完全不同） ^mdc-019808f78b
- Busbar 采用 SIEMENS 2500A 封闭式母线
- TOU 每路 250A 为单机柜供电
- BESS 连接方式：Grid → BESS → DC45（Power Zone 负责） ^mdc-730fd2f50a
...
| **IT Zone（DC45 集装箱）** | **无内部冗余** — 单台 DC45 独立运行，不含内部 N+1/2N ^mdc-1cd111257c |
| **MDC 系统级**           | 多台 DC45 并联 → 系统级冗余（N / N+1 / 2N 由集装箱数量决定） ^mdc-49aa6fbf3e |
| **OEM 机柜**            | 客户自备机柜无内置冗余，需通过系统级 MDC 冗余实现高可用          |

> DC45 是一台完整的集装箱设备，不存在"多增加几个服务器"来增加冗余的概念。\ ^mdc-5a0f8fc07d
> 如需更高可靠性，通过增加 DC45 集装箱数量实现。 ^mdc-67c0285bcb

***

## 8. 扩展逻辑

- 可与 AC40 混合构建 MDC ^mdc-ce3debe847
- 支持横向扩展（增加 DC45 数量） ^mdc-dd5038d990
...
| -------- | ----------------- | ------ | ----- | -------- | ------- | --------------- | --------------------- |
| **AC40** | EATON 9395XR-600  | 4 UPM  | 150kW | 600kW    | ~7.9kW  | **外置（客户自备）**    | ~10 分钟（客户自备 2×93LiG2） ^mdc-713ef544f7 |
| **AC45** | EATON 9395XR-600  | 4 UPM  | 150kW | 600kW    | ~7.9kW  | 内置（专用电力舱），UL 合规 | ~20 分钟（内置 2×93LiG2） ^mdc-cc1e001aba |
| **DC45** | EATON 9395XR-1500 | 10 UPM | 150kW | 1500kW   | ~46.9kW | 内置，UL 合规        | ~8 分钟（内置 3×93LiG2） ^mdc-173265b658 |
